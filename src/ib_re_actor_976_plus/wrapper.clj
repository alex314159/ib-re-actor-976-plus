...
;
;    (connectAck [this]
;      (dispatch-message cb {:type :connect-ack}))
;
;    ;;;; Market Data
...
;                            :implied-volatility impliedVolatility :opt-price optPrice :pv-dividend pvDividend
;                            :und-price undPrice :delta delta :gamma gamma :theta theta :vega vega}))
;                            ;:value {:field (translate :from-ib :tick-field-code field)
...
;      (dispatch-message cb {:type :tick-efp :ticker-id tickerId
...
;      (dispatch-message cb {:type :next-valid-id :order-id orderId}))
;
...
;      (dispatch-message cb {:type :pnl-single :request-id reqId :pos pos :daily-pnl dailyPnL :unrealized-pnl unrealizedPnL :realized-pnl realizedPnL :value value}))
;
;
;    ;;; Contract Details
...
;      (dispatch-message cb {:type :exec-details :request-id requestId :contract contract :execution execution}))
;                            ;:value {:contract contract      ;(->map contract)
;                            ;        :value (->map execution)}
;
;    (execDetailsEnd [this requestId]
;      (dispatch-message cb {:type :exec-details-end :request-id requestId}))
;
...
;      (dispatch-message cb {:type :update-mkt-depth :ticker-id tickerId :position position :operation operation :side side :price price :size size}))
...
;      (dispatch-message cb {:type :update-mkt-depth-l2 :ticker-id tickerId :position position :market-maker marketMaker :operation operation :side side :price price :size size :is-smart-depth isSmartDepth}))
;
;    ;;; News Bulletin
;    (updateNewsBulletin [this msgId msgType message origExchange]
;      (dispatch-message cb {:type :update-news-bulletin  :msg-id msgId  :msg-type msgType :message message :orig-exchange origExchange}))
...
;      (dispatch-message cb {:type :historical-data :request-id requestId :bar bar}))
...
;      (dispatch-message cb {:type :historical-data-update :request-id requestId :bar bar}))
;
...
;      (dispatch-message cb {:type :real-time-bar :request-id requestId
